...
from . import tcp
from . import util
from .securecarla import SecureCarla
...
class CarlaClient(object):
    ...
    def read_data(self, adversarial=False):
        """
	...
	if adversarial==False:
            temp =  self._parse_raw_sensor_data(raw_sensor_data)
            return pb_message, self._parse_raw_sensor_data(raw_sensor_data)
	else:
	    return self._secure_carla.inject_adversarial(pb_message, self._parse_raw_sensor_data(raw_sensor_data))
